...
# to stdout — the queue file is the source of truth and /voca queue surfaces it.
#
# Recursion guard: if VOCA_HOOK_RUNNING is set, this is a nested claude
# session spawned by the extractor — exit immediately.

set -uo pipefail

if [[ "${VOCA_HOOK_RUNNING:-0}" == "1" ]]; then
  exit 0
fi
...
LOG="$HOOK_LOG"
ASYNC="$HOOK_DIR/voca-extract-async.sh"
